...
import { speechSynthesis } from "@/modules/read/api/speechSynthesis";
import { translateArticle } from "@/modules/read/api/translateArticle";
...
export default function ReadScreen() {
  ...
  // Premium gate for speak
  const handleSpeak = async () => {
    if (!isPremium) {
      Alert.alert(
        "升级高级会员",
        "朗读功能仅限高级会员使用，立即升级解锁更多特权？",
        [
          { text: "取消", style: "cancel" },
          { text: "升级", onPress: () => router.push("/premium") },
        ],
      );
      return;
    }
    ...
    try {
      ...
      const fullText = [articleData.title, ...articleData.paragraphs].join(
        "\n",
      );
      await speechSynthesis({
        articleId: params.id || "demo",
        text: fullText,
      });
    } catch (error) {
      console.error("Speech synthesis error:", error);
      Alert.alert("朗读失败", "请稍后重试");
    } finally {
      setIsSpeaking(false);
    }
  };

  // Premium gate for translate
  const handleTranslate = async () => {
    if (!isPremium) {
      Alert.alert(
        "升级高级会员",
        "翻译功能仅限高级会员使用，立即升级解锁更多特权？",
        [
          { text: "取消", style: "cancel" },
          { text: "升级", onPress: () => router.push("/premium") },
        ],
    ...
    try {
      const fullText = articleData.paragraphs.join("\n\n");
      const result = await translateArticle({
        articleId: params.id || "demo",
        text: fullText,
        targetLanguage: "zh", // Default target language
      });
      ...
      <View style={styles.readerToolbarWrap}>
        ...
        <TouchableOpacity style={styles.speakButton} onPress={handleSpeak}>
          <MaterialIcons
            ...
            color={colors.onPrimary}
          />
          <Text style={styles.speakLabel}>{isSpeaking ? "停止" : "朗读"}</Text>
        </TouchableOpacity>
        <View style={styles.toolbarGroup}>
          <TouchableOpacity style={styles.toolButton} onPress={handleTranslate}>
            <MaterialIcons
              name={translatedText ? "close" : "translate"}
              size={22}
              color={translatedText ? colors.primary : colors.onSurfaceVariant}
            />
            <Text
              style={[
                styles.toolLabel,
                translatedText && { color: colors.primary },
              ]}
            >
              {translatedText ? "关闭" : "翻译"}
            </Text>
          </TouchableOpacity>
          <TouchableOpacity style={styles.toolButton}>
            <MaterialIcons

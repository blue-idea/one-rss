...
## 六、CodeRabbit/Gemini Review 问题处理流程

当 CodeRabbit 或 Gemini 发现问题时，按照以下流程处理：

1. **分析问题严重性**：
   - P0（安全漏洞、逻辑错误）：必须修复
   - P1/P2（代码风格、潜在问题）：评估是否影响功能

2. **如果需要修复**：
   - 直接在当前 worktree 中修复
   - 运行 `lint --fix` 自动修复可修复的问题
   - 手动修复需要代码改动的问题
...
4. **提交推送**：
   ```bash
   git add . && git commit -m "fix(<task>): <修复描述>" && git push
   ```
...
1. **尽量减少人工介入**：除非触发人工介入条件，否则全自动执行
2. **Code Review 优先修复**：AI reviewer 发现的问题及时修复后再合并
3. **质量门禁不可绕过**：lint/typecheck/test 必须全部通过才能合并
4. **worktree 隔离**：每个任务使用独立 worktree，避免分支污染
5. **网络错误自动重试**：代理问题自动处理，重试上限 3 次
